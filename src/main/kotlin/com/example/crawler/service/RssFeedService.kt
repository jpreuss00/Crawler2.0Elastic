...
class RssFeedService(@Autowired val rssFeedRepository: RssFeedRepository, @Autowired val articleService: ArticleService) {
    ...
    fun saveRssFeedToDB(items: Items) = articleService.storeArticles(items.channel.item.map { ArticleModel(it.guid, it.title, it.category, it.pubDate, it.description, it.link) })
}
